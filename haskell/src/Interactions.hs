...
isAtLocation state locationName =
  case findLocation locationName of
    Nothing -> False
    Just loc -> location state == loc

give :: GameState -> String -> String -> IO GameState
give state "homeless" "cigarettes" =
  if isAtLocation state "Homeless Bench" && hasItem state "Cigarettes"
    then do
      ...
      newState <- addItem newState "Weird Box"
